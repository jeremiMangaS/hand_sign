...
with HandLandmarker.create_from_options(options) as landmarker :
    while cap.isOpened() :
        ...
        # logic
        # hand_landmarker_result = landmarker.detect(mp_image_frame)
        # frame_timestamp_ms = int((time.time() - start_time) * 1000 / cv.getTickFrequency())
        frame_timestamp_ms = int(time.time() * 1000)
        landmarker.detect_async(mp_image_frame, frame_timestamp_ms)

        # if landmarker.hand_landmarks :
        #     for hand_landmarks in landmarker.hand_landmarks :
